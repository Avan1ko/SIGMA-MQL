...
class SumTree:
    ...
    def batch_sample(self, batch_size: int):
        ...
        idxes = np.zeros(batch_size, dtype=np.int32)
...
class LocalBuffer:
    ...
    def __init__(self, actor_id: int, num_agents: int, map_len: int, init_obs: np.ndarray,
        ...
        self.obs_buf = np.zeros((capacity+1, num_agents, *obs_shape), dtype=np.bool_)
        ...
        self.comm_mask_buf = np.zeros((capacity+1, num_agents, num_agents), dtype=np.bool_)
