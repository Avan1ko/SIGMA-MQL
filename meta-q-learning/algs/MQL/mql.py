...
class MQL:
    ...
    def __init__(self, 
        ...
        '''
            ...
            enable_beta_obs_cxt:  decide whether to concat obs and ctx for logistic regression
            lam_csc: logistic regression reg, smaller means stronger reg
        '''
        self.actor = actor
    ...
    def train_cs(self, task_id = None, snap_buffer = None, train_tasks_buffer = None, adaptation_step = False):
        ...
        with torch.no_grad():   
            ...
            if self.enable_beta_obs_cxt == True:
                snap_ctxt = torch.cat([pos_pxx, self.actor.get_context_feats(pos_act_rew_obs)], dim = -1).cpu().data.numpy()
                neg_ctxt = torch.cat([neg_xx, self.actor.get_context_feats(neg_act_rew_obs)], dim = -1).cpu().data.numpy()

            else:
                snap_ctxt = self.actor.get_context_feats(pos_act_rew_obs).cpu().data.numpy()
                neg_ctxt = self.actor.get_context_feats(neg_act_rew_obs).cpu().data.numpy()
    ...
    def get_propensity(self, cs_model, curr_pre_act_rew, curr_obs):
        ...
        with torch.no_grad():
            ...
            # batch_size X context_hidden 
            if self.enable_beta_obs_cxt == True:
                ctxt = torch.cat([curr_obs, self.actor.get_context_feats(curr_pre_act_rew)], dim = -1).cpu().data.numpy()

            else:
                ctxt = self.actor.get_context_feats(curr_pre_act_rew).cpu().data.numpy()
